...
/*                                                                          */
/*                 (Copyright by Aaron Wewior, 11/2019)                     */
/*                                                                          */
/*--------------------------------------------------------------------------*/
...
/* 
 ...
 Output: difference between input images, as well as average and maximum
				 absolute difference
*/
...
float  mean;								 /* average absolute difference */
float  max=0; 							 /* maximum absolute difference */
float  min=255;							 /* minimum absolute difference */
char   comments[1600];       /* string for comments */

printf ("\n");
printf ("ABSOLUTE DIFFERENCE BETWEEN TWO IMAGES\n\n");
printf ("*****************************************************\n\n");
...
printf ("output image(abs. difference (pgm):     ");
read_string (out);
printf ("\n");
...
for (i=1; i<=nx; i++)
	for (j=1; j<=ny; j++)
	{
// 		u[i][j] = fabs(f1[i][j] - f2[i][j]);
		u[i][j] = fabs(f2[i][j] - f1[i][j]);
		if (u[i][j] > max)
			max = u[i][j];
		if (u[i][j] < min)
			min = u[i][j];	
...
printf("average absolute difference: %8.4f\n", mean);
printf("maximum absolute difference: %8.4f\n", max);
printf ("\n");

/* rescale result to [0,255] */
	for (i=1; i<=nx; i++)
		for (j=1; j<=ny; j++)
        u[i][j] = 255.0 * u[i][j] / (max-min);
...
comment_line (comments, "# average absolute difference: %8.4f\n",mean);
comment_line (comments, "# maximum absolute difference: %8.4f\n",max);
